...
export const generateRss2 = async (): Promise<string> => {
  ...
  const newArrival = Array.from(h2Doms).find((element) =>
    (element as Element).innerText == "新着情報【新規掲載】"
  ) as Element;
  ...
  const entries = pDoms.map((element) => {
    ...
    const [, yearStr, monthStr, dayStr, title] =
      /(\d{4})年(\d+)月(\d+)日\s*(.*)$/.exec(text) || ["", "", "", "", ""];
    const [year, month, day] = [yearStr, monthStr, dayStr].map((val) =>
      parseInt(val, 10)
    );
